...
- `responder_faq`: lógica de umbral y verificación, con `buscar` y el LLM simulados.
...
# --- responder_faq: umbral y verificación (con dobles) --------------------
...
class FakeLlm:
    ...
    def create_chat_completion(self, **kw):
        self.calls += 1
        # 1ª llamada = verificación SI/NO; 2ª = generación
...
check(r.categoria == Categoria.CON_HUMANO and r.metodo == "rag_sin_fundamento",
      "la verificación dice NO -> con_humano/rag_sin_fundamento")
...
check(r.categoria == Categoria.FAQ_ESTATICA and r.metodo == "rag" and r.fuente,
      "verificación SI + respuesta fundada -> faq_estatica con fuente citada")
...
check(r.categoria == Categoria.FAQ_ESTATICA and r.metodo == "rag",
      "respuesta válida con aclaración 'no se menciona' al final -> NO se deriva")
