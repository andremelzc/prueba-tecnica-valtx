...
- Cómo se expone en una URL (ngrok) y por qué: [`DEPLOY.md`](DEPLOY.md)
...
(CPU), **descarga el modelo GGUF (~2.4 GB)** y compila el frontend. Cuando
...
Hay que esperar a que loguee `LLM cargado` e `Índice RAG: {...}` (~20-30 s la
primera vez).
...
El `.env` ya apunta a `http://localhost:8000`. Abrir **http://localhost:5173**.
...
- **Caja de texto**: consulta al backend real. Cada categoría tarda distinto:
  ...
  | Consulta | Categoría | Tiempo aprox. (CPU) |
  |---|---|---|
  ...
  Con **"Ver detalle"** en cada respuesta se ve: fuente citada, score de
  confianza, método de clasificación (`regla`, `llm`, `rag_baja_confianza`,
  etc.), si fue duplicado, y el resumen para el revisor.
...
- docker-compose.yml
- DEPLOY.md                cómo se expone en una URL (ngrok) y por qué
...
- **Latencia:** todo corre en CPU. Las consultas `faq_estatica` hacen 2 llamadas
  al LLM (verificación de relevancia + generación), ~30-45 s. Es esperado; el
  desglose de tiempos está en [`backend/README.md`](backend/README.md).
